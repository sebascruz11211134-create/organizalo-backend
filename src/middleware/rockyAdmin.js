...
module.exports=function(req,res,next) {
  try {
    ...
    if(!user || (user.empresa_id||user.id)!==claim.empresaId || !['admin','superadmin','gerencia'].includes(user.rol)) return res.status(403).json({error:'Se requiere administración de la empresa.'});
